...
module hsync(
    ...
    // Генерация сигнала горизонтальной синхронизации
    always @ (posedge clk) begin
    ...
    // Генерация сигнала бланкировки (высокий уровень во время отображения)
    always @ (posedge clk) begin
        if (counter < DISPLAY)
            blank_out <= 0;
        else
    ...
    // Генерация сигнала новой строки
    always @ (posedge clk) begin
        if (counter == 0)
            newline_out <= 1;
        else
...
module vsync(
    ...
    // Генерация сигнала вертикальной синхронизации
    always @ (posedge newline_in) begin
        if (counter < (DISPLAY + FRONTPORCH))
            vsync_out <= 1;
        else if (counter >= (DISPLAY + FRONTPORCH) && counter < (DISPLAY + FRONTPORCH + SYNC))
...
module data(
    ...
    // Линейное преобразование для масштабирования значений f0-f15
...
module VGA_generator(
    ...
    // Загрузка данных FFT при завершении
    always @ (posedge clk) begin
        if (done) begin
            f0_reg <= f0;
            f1_reg <= f1;
